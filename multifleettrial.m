...
%   Select input file and sheet
    ...
    ACchar      =   xlsread(filename,1,'B29:C37');            
    cost        =   ACchar(1,:);
    LF          =   ACchar(2,:);
    nseats      =   ACchar(3,:);
    speed       =   ACchar(5,:);
...
%%  Objective Function
        Profit_X               =   zeros(Nodes*Nodes,1);                 %Prepare a matrix of OF coefficients
        Profit_W               =   zeros(Nodes*Nodes,1);
        Cost_Z                 =   zeros(Nodes*Nodes,ACtype);
         for k = 1:ACtype
             Cost_Z(:,k)       =   reshape(cost(k).*distance'.*nseats',Nodes*Nodes,1);
         end
        Cost_Z1                =   reshape(Cost_Z,Nodes*Nodes*ACtype,1);
        
        obj                    =   [Profit_X;Profit_W;Cost_Z1];
        lb                     =   zeros(DV,1);
        ub                     =   inf(DV,1);
