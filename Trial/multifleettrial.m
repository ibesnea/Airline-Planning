...
%   C2: Transfer passenger constraint
    ...
    for i = 1:Nodes
        for j = 1:Nodes
            ...
            cplex.addRows(0,C2,demand(i,j)*gi(i)*gj(j),sprintf('Transfer Pax %d_%d_%d',i,j));
        end
    end
...
%   C3: Capacity verification constraints
    ...
    for i=1:Nodes
        for j= 1:Nodes
            ...
            for k = 1:ACtype
                C3(Zindex(i,j,k,Nodes)) = -nseats(k)*-LF(k);
            end
...
%   C5: Aircraft utilization
    distance = reshape(distance,Nodes,Nodes);
    for k = 1:ACtype
        C5 = zeros(1,DV);
        for i = 1:Nodes
            for j = 1:Nodes
                C5(Zindex(i,j,k,Nodes)) = (distance(i,j)/speed(k)+LTO(k));
            end
        end
...
%%  Postprocessing
  %Store direct results
    ...
    if status == 101 || status == 102 || status == 105
        ...
        for k = 1:ACtype
            sol.Freq (:,:,k)= round(reshape(cplex.Solution.x(Zindex(1,1,k,Nodes):Zindex(Nodes, Nodes, k,Nodes)), Nodes, Nodes))';
        end
        sol.DirFlow(:,:)= round(reshape(cplex.Solution.x(Xindex(1,1,Nodes):Xindex(Nodes, Nodes,Nodes)), Nodes, Nodes))';
        sol.ConFlow(:,:)= round(reshape(cplex.Solution.x(Windex(1,1,Nodes):Windex(Nodes, Nodes,Nodes)), Nodes, Nodes))';
    end
